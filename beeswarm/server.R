...
#		test = paste(test, infile)
	...
	outputs = list(xdat, clin_dat, Color_Var, Reactive_Groups)
	return(outputs)
  }

  plotBeeSwarm <- function(xdat, Color_Var, Reactive_Groups) {
...
#		labs(title = input$Color) +
  ...
  # FUNCTION FOR CREATING BEESWARM PLOT
  plotInput <- reactive({
	inputs = processFunc()
	xdat = inputs[[1]]
	clin_dat = inputs[[2]]
	Color_Var = inputs[[3]]
	Reactive_Groups = inputs[[4]]

	swarm_plot = plotBeeSwarm(xdat, Color_Var, Reactive_Groups)
	swarm_plot
  })
  ...
  output$downloadTable <- downloadHandler(
	filename = "test.xlsx",
	content = function(file) {
#		write.table(tableInput(), file, sep = "\t", quote = FALSE)
		write.xlsx(tableInput(), file)
	}
  )
